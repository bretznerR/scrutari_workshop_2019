...
Scrutari.Client.prototype.$area = function (name, action) {
    var $area = $("[data-scrutari-area='" + name + "']");
    if (action) {
        switch(action) {
            case 'show':
                this.show ($area);
...
Scrutari.Client.prototype.$block = function (name, action) {
    var $block = $("[data-scrutari-block='" + name + "']");
    if (action) {
        switch(action) {
            case 'show':
                this.show ($block);
...
Scrutari.Client.prototype.$button = function (name, action, option) {
    var $button = $("[data-scrutari-button='" + name + "']");
    if (action) {
        switch(action) {
            case 'on':
            ...
            case 'hide':
                ...
                break;
            case 'onclick':
                $("#" + this.clientId).on("click", "[data-scrutari-button='" + name + "']", option);
                break;
        }
    }
    return $button;
...
Scrutari.Client.prototype.$count = function (name, action, value) {
    var $count = $("[data-scrutari-count='" + name + "']");
    if (action) {
        switch(action) {
            case 'update':
                if (!value) {
                ...
                $count.find("[data-scrutari-role='value']").html(this.formatNumber(value));
                break;
        }
    }
    return $count;
...
Scrutari.Client.prototype.$panel = function (name, action) {
    var $panel = $("[data-scrutari-panel='" + name + "']");
    if (action) {
        switch(action) {
            case 'show':
                this.show ($panel);
...
Scrutari.Client.prototype.$title = function (name, action) {
    var $title = $("[data-scrutari-title='" + name + "']");
    if (action) {
        switch(action) {
            case 'show':
                this.show ($title);
...
Scrutari.Client.Result.show = function (client, scrutariResult, searchOrigin) {
    ...
    if (searchOrigin === "mainsearch") {
        ...
        if (client.currentHistoryName) {
            client.$block(client.currentHistoryName).find("[data-scrutari-role='subsearch']").text(subsearchText);
        }
    }
    if (ficheCount === 0) {
...
Scrutari.Client.Result.addToHistory = function (client, scrutariResult) {
    ...
    if (client.currentHistoryName) {
        ...
        $historyBlock.find("[data-scrutari-role='subsearch']").empty();
        client.show ($historyBlock.find("[data-scrutari-role='remove']"));
    }
...
Scrutari.Client.Result.loadHistory = function (client, historyName) {
    ...
    if (historyScrutariResult) {
        ...
        client.hide($historyBlock.find("[data-scrutari-role='remove']"));
...
Scrutari.Client.Ui.filterChange = function (client) {
    ...
    var $filterValue = $filterFicheCount.find("[data-scrutari-role='value']");
    ...
    var $filterTitles = $filterFicheCount.find("[data-scrutari-role='titles']");
    ...
    function _updateState(type) {
        var $stat = $("[data-scrutari-stat-type='" + type + "']");
        var testCount = false;
        if (filterFicheCount === -1) {
            filterFicheCount = 0;
            testCount = true;
        }
        $stat.each(function (index, element) {
            ...
            if (testCount) {
                filterFicheCount += ficheCount;
            }
            var $statTitle = $el.parents("[data-scrutari-role='stat-text']");
...
Scrutari.Client.Ui.initClicks = function (client) {
    $("#" + client.clientId).on("click", "[data-scrutari-button]", function () {
        var $this = $(this);
    ...
    function _categoryTab ($button) {
        ...
        $("[data-scrutari-role='category-content']").each(function (index, element) {
            var $element = $(element);
        ...
        $("[data-scrutari-role='category-tab']").each(function (index, element) {
            var $element = $(element);
            if ($element.data("scrutariCategory") === categoryName) {
